 ...
 * =========================== */
(function heroSlider(){
  // keep desktop-only behavior; remove this line if you want it on mobile too
  if (window.matchMedia('(max-width: 680px)').matches ||
      window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  const slider = document.querySelector('.box-stack-wrapper');
  if (!slider) return;

  // ⬇️ scope to the hero only
  const boxes = slider.querySelectorAll('.box');
  const dots  = slider.querySelectorAll('.dots .dot');
  ...
  function start() {
    stop();
    timer = setInterval(() => { index = (index + 1) % 3; render(); }, 3000);
  }
  function stop() { if (timer) { clearInterval(timer); timer = null; } }

  // clicking a dot jumps and restarts loop
...
// === INDUSTRIES STACK CAROUSEL ===
(function industriesCarousel() {
  const cards = document.querySelectorAll(".industry-card");
  const dots  = document.querySelectorAll(".industries-dots .dot");
  ...
  function startAutoPlay() {
    interval = setInterval(next, 5000); // 5s cycle
  }
...
(function productsCarousel(){
  const track = document.querySelector(".products-carousel .carousel-track");
  if (!track) return;
  ...
  function next(){ index = (index + 1) % cards.length; updateCards(); }
  function prev(){ index = (index - 1 + cards.length) % cards.length; updateCards(); }
  ...
  track.addEventListener("click", e => {
    if (e.target.closest(".left")) prev();
    if (e.target.closest(".right")) next();
    stopAuto(); startAuto();
  });
